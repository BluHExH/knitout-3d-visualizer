...
export function compileKnitoutJs(source: string): CompileResult {
  try {
    const knitout = { Writer: KnitoutWriter }
    const wrapped = `
"use strict";
const __logs = [];
const console = {
...
const __result = (function(knitout) {
  ${source}
})(knitout);
if (typeof __result === 'string' && __result.includes('knitout')) return __result;
if (__result && typeof __result.write === 'function') return __result.write();
if (__logs.length) {
  const joined = __logs.join('\n');
  if (joined.includes(';!knitout') || joined.includes('knit ')) return joined + '\n';
}
throw new Error(
  'JS must create knitout.Writer, call ops, then return k.write() (or console.log knitout lines).'
);
`
    const fn = new Function(wrapped)
    const out = fn() as string
    if (!out || typeof out !== 'string') {
      return { ok: false, error: 'Writer produced empty output' }
    }
    let text = out.trim() + '\n'
    if (!text.startsWith(';!knitout')) {
